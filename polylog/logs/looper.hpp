...
namespace polylog
{
        ...
        void threadEntry()
        {
            while (1)
            {
                ...
                // 为互斥锁设置一个生命周期，当缓冲区交换完毕后就解锁（并不对数据的处理过程加锁保护）
                {
                    ...
                    // 退出标志被设置且生产缓冲区已无数据，这时候再退出,否则可能会造成生产缓冲区中有数据，但是没有被完全处理
    ...
    public:
        ...
        AsyncLooper(const Functor &cb, AsyncType loop_type = AsyncType::ASYNC_SAFE) : _stop(false), _looper_type(loop_type),
